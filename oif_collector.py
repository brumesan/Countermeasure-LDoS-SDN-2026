#!/usr/bin/env python3
# -*- coding: utf-8 -*-
""" Ryu + pure Online Isolation Forest (OIF). Applied only when the port state is classified as abnormal. """
...
class OIFCollector(app_manager.RyuApp):
    OFP_VERSIONS = [ofproto_v1_3.OFP_VERSION]

    def __init__(self, *args, **kwargs):
        super(OIFCollector, self).__init__(*args, **kwargs)
        ...
        self.NORMAL_CONSEC_REQUIRED = 50
...
#        timestamp = time.strftime("%Y%m%d_%H%M%S")
    ...
    def _detect_and_record(self, dpid, port, rate_in, rate_out, pnf, ppnf):
        # === Automatic time-based warm-up transition ====
        if self.in_warmup_mode and (time.time() - self.warmup_start_time) >= self.WARMUP_DURATION:
            self._save_warmup()
        ...
        if self.in_warmup_mode:
            ...
            if self.oif is not None and dpid in allowed_switches:
            ...
            if port_state == "normal":
                status = "NORM"
                alert_type = "LEARN_ALWAYS" if dpid in allowed_switches else "NORM"
